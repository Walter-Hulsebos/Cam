...
namespace Lean.Common
{
	...
	public struct LeanScreenQuery
	{
		...
		private void DoRaycast3D(Camera camera, Vector2 screenPosition, ref Component bestResult, ref float bestDistance, ref Vector3 bestPosition)
		{
			...
			//Debug.DrawRay(start: ray.origin, dir: ray.direction * 50, color: Color.red, duration: 1f);
			...
			Debug.DrawLine(start: ray.origin, end: bestPosition, Color.cyan, duration: 1f);
			
			//Draw X at the hit location
			Debug.DrawLine(start: bestPosition + (Vector3.up      * 0.5f), end: bestPosition - (Vector3.up      * 0.5f), Color.yellow, duration: 1f);
			Debug.DrawLine(start: bestPosition + (Vector3.left    * 0.5f), end: bestPosition - (Vector3.left    * 0.5f), Color.yellow, duration: 1f);
			Debug.DrawLine(start: bestPosition + (Vector3.forward * 0.5f), end: bestPosition - (Vector3.forward * 0.5f), Color.yellow, duration: 1f);

			if (bestResult != null)
			{
				Debug.Log($"Hit {bestResult.name} at {bestPosition}");	
			}
		}
